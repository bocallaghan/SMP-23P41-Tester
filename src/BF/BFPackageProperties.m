...
@implementation BFPackageProperties
...
- (id) init
{
    if ((self = [super init]))
    {
        self.classMetaData = [BFPackageProperties metaData];
        [self setEntityDelegate:(SUPEntityDelegate*)[BFPackageProperties delegate]];
    }
    return self;    
}
- (void)dealloc
...
+ (BFPackageProperties*)getInstance
{
     BFPackageProperties* me = [[BFPackageProperties alloc] init];
    [me autorelease];
    return me;
}
...
static SUPEntityMetaDataRBS* BFPackageProperties_META_DATA;

+ (SUPEntityMetaDataRBS*)metaData
{
    if (BFPackageProperties_META_DATA == nil) {
		BFPackageProperties_META_DATA = [[BFPackagePropertiesMetaData alloc] init];
	}
	
	return BFPackageProperties_META_DATA;
}
...
+ (id)allocInitialTraceRequest:(NSString*)method withSkip:(int32_t)skip withTake:(int32_t)take withSql:(NSString*)sql withValues:(SUPObjectList*)values
{
    ...
    if (level == CFSCTraceLevel_DETAIL)
    {
        ...
        request = [[SUPCFSCTraceDetailRequest alloc] initWithParams:@"BFPackageProperties" :method :nil:nil:[NSDate date] :nil :0
                                                                   :nil :NO :NO :NO :NO :NO :sql :params];
    } else if (level == CFSCTraceLevel_NORMAL)
    {
        request = [[SUPCFSCTraceRequest alloc] initWithParams:@"BFPackageProperties"  :method :nil :nil  :[NSDate date] :nil :0 :params];
    }
    return request;
}
...
+ (BFPackageProperties*)find:(NSString*)id_
{
    SUPCFSCTraceRequest* request = nil;
    NSString *msg = @"success";
    if ([SUPCFSCTrace isEnabled])
    {
        request = [[SUPCFSCTraceRequest alloc] initWithParams:@"BFPackageProperties" :@"find" 
            :nil :[self metaData].synchronizationGroup  :[NSDate date] :nil :0 :[SUPStringUtil toString_object:id_]];
    } 
    BFPackageProperties* result = nil;
    @try {
         SUPObjectList *keys = [SUPObjectList getInstance];
         [keys add:id_];
         result = (BFPackageProperties*)[(SUPEntityDelegate*)([[self class] delegate]) findEntityWithKeys:keys];
         return result;
     }
     @catch (NSException *e) {
        msg = [NSString stringWithFormat:@"fail in BFPackageProperties--find: %@:%@", [e name], [e reason]];        
...
{
    return (SUPObjectList*)[(SUPEntityDelegate*)([[self class] delegate])  findWithQuery:query:[BFPackageProperties class]];
}
...
+ (BFPackageProperties*)load:(NSString*)id_
{
    return (BFPackageProperties*)[(SUPEntityDelegate*)([[self class] delegate]) load:id_];
}
#pragma mark -
...
- (BFPackageProperties*)getDownloadState
{
    return (BFPackageProperties*)[self i_getDownloadState];
}

- (BFPackageProperties*) getOriginalState
{
    return (BFPackageProperties*)[self i_getOriginalState];
}
...
{
   return [BFLogRecordImpl findByEntity:@"PackageProperties":[self keyToString]];
}
...
static SUPEntityDelegate *g_BFPackageProperties_delegate = nil;

+ (SUPEntityDelegate *) delegate
{
	@synchronized(self) {
		if (g_BFPackageProperties_delegate == nil) {
			g_BFPackageProperties_delegate = [[SUPEntityDelegate alloc] initWithName:@"BFPackageProperties" clazz:[self class]
				metaData:[self metaData] dbDelegate:[BFSMP_TesterDB delegate] database:[BFSMP_TesterDB instance]];
		}
	}
	
	return [[g_BFPackageProperties_delegate retain] autorelease];
}
...
+ (SUPObjectList*)fromJSONList:(SUPJsonArray*)jsonArray
{
    ...
    {
        BFPackageProperties* inst = [[BFPackageProperties alloc] init];
...
+ (SUPJsonArray*)toJSONList:(SUPObjectList*)instanceList
{
    ...
    for (BFPackageProperties* inst in instanceList)
    {
